...
inputWorkbook = xlrd.open_workbook("Tüm Edit.xls")
...
for t in range(inputWorksheet.nrows-1):
    ...
    attendee.append(inputWorksheet.cell_value(t,2))
...
for x in range(len(attendees)):
    try:
        if attendees[x][1] == "info@hmaksu.com":
            if attendees[x][0] == attendees[x+1][0]:
                k += 1
            else:
                if k == 0:
            ...
            if attendees[x][0] == attendees[x+1][0] or attendees[x][1] == attendees[x+1][1]:
                k += 1
            else:
                if k == 0:
                    attendees[x].append(int(attendees[x][-1]))
...
attendees_vol2.sort()
outworkbook = xlsxwriter.Workbook("Tüm Edit Vol2.xlsx")
